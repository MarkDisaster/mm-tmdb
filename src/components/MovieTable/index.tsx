    ...
    <CTableBody>
        <CTableRow>
          ...
          {movies?.map((movie, index) => {
            return (
              <CTableRow key={index} className='mb-2'>
                <CTableDataCell>checkbox</CTableDataCell>
                <CTableDataCell className='p-0'>
                  {movie.poster_path
                    ? <CImage align="start" src={`${TMDB_IMG_URI}${movie.poster_path}`} width={50} />
                    : <CIcon icon={cilImage} height={50} className='text-secondary' />
                  }
                </CTableDataCell>
